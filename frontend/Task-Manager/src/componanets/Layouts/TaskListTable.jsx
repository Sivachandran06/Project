...
const TaskListTable =({tabelData})=>{
     ...
     const getPriorityBadgeColor = (priority) =>{
        switch(priority){
            ...
            default: return 'bg-green-100 text-gray-500 border border-gray-200';
        }
     };
    return(
            ...
            <tbody>
                {tabelData.map((task)=>(
                    <tr key ={task._id} className="border-t border-gray-200">
                        ...
                        <td className="py-4 px-4">
                            <span className={`px-2 py-1 test-xs rounded inline-block ${getPriorityBadgeColor(task.priority)}`}>{task.priority}</span>
                        </td>
                        <td className="py-4 px-4 text-gray-700 text-[13px] text-nowrap hiddedn md:table-cell">{task.createdAt ? moment(task.createdAt).format('Do MMM YYYY'):'N/A'}</td>
                    </tr>
                ))}
            </tbody>
        </table>
